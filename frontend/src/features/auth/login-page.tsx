    ...
    <main className="grid min-h-dvh lg:grid-cols-[0.95fr_1.05fr]">
      <section className="login-story relative isolate flex flex-col justify-between overflow-hidden bg-sidebar p-7 text-sidebar-foreground sm:p-10 lg:p-14 xl:p-16">
        <div className="relative z-10">
          <div className="inline-flex rounded-xl border border-white/15 bg-white px-4 py-3 shadow-sm">
            <img
          ...
          <p className="mt-3 text-[10px] font-medium tracking-[0.18em] text-white/45">
            E-PROCUREMENT & DIGITAL TENDER MANAGEMENT
          </p>
        </div>

        <div className="relative my-12 hidden max-w-lg lg:block lg:my-16">
          <span className="mb-7 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs tracking-wide text-white/80">
            <span className="size-1.5 rounded-full bg-teal-300" />
            CONNECTING PROCUREMENT & CARE
          </span>

          <h1 className="text-4xl font-semibold leading-[1.12] tracking-tight sm:text-5xl xl:text-6xl">
            Better procurement.
            <br />
          ...
          <p className="mt-6 max-w-sm text-base leading-relaxed text-white/70">
            One portal for hospital teams and trusted suppliers. Supporting the
            supplies, equipment, and services that patient care depends on.
          </p>

          <div className="mt-10 hidden border-t border-white/15 pt-7 lg:block">
            <p className="mb-5 text-[10px] font-medium tracking-[0.2em] text-white/50">
              A CONNECTED PROCUREMENT JOURNEY
            </p>
            ...
            <div className="flex items-start justify-between gap-3 text-sm">
              {[
                ...
                <div key={title}>
                  <Icon
                    className="mb-3 size-5 text-teal-200"
                    aria-hidden="true"
                  />
      ...
      <section className="flex flex-col justify-center bg-card px-6 py-10 sm:px-12 lg:px-16">
        <div className="mx-auto w-full max-w-[420px]">
          <div className="mb-8">
            <p className="mb-3 text-xs font-semibold tracking-[0.18em] text-primary">
              BSH E-PROCUREMENT PORTAL
            </p>
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Sign in to your hospital or vendor workspace.
            </p>
          </div>
          ...
          <div className="mt-8 border-t pt-6">
            <div className="mb-1 flex items-center justify-between">
              <h3 className="text-sm font-semibold">Demo Access</h3>

              <span className="rounded bg-accent px-2 py-1 text-[10px] font-semibold tracking-wide text-accent-foreground">
                CLIENT DEMO
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {demoAccounts.map(({ account, role }) => (
                <Button
                  ...
                  className={
                    role === "vendor"
                      ? "col-span-2 h-10 justify-between px-3"
                      : "h-11 justify-between gap-2 whitespace-normal px-3 text-xs"
                  }
                  disabled={busy}
                  onClick={() =>
                    signIn({
                ...
                >
                  <span className="min-w-0 text-left">{roleLabels[role]}</span>
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Demo password:{" "}
              <code className="font-medium text-foreground">Demo@12345</code>
            </p>
          </div>

          <p className="mt-8 flex items-center gap-2 text-xs leading-relaxed text-muted-foreground">

...
function WorkspaceNavigation({
  ...
  return (
    ...
    >
      <p className="mb-4 px-3 text-[10px] font-medium tracking-[0.15em] text-sidebar-foreground/50">
        {config.subtitle}
      </p>
        ...
        {navigation.map(({ path, label, icon: Icon }) => (
          <li key={path}>
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "flex min-h-11 items-center gap-3 rounded-lg px-3 text-[13px] transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sidebar-accent-foreground",
                  isActive
...
function SidebarFooter() {
  return (
    <div className="mx-5 mb-5 shrink-0 rounded-lg border border-sidebar-border p-3 text-sidebar-foreground/65">
      ...
      <div className="flex min-h-dvh flex-col lg:pl-64">
        <header className="sticky top-0 z-20 flex min-h-20 items-center justify-between gap-3 border-b bg-card px-4 sm:px-8">
          ...
          <div className="flex shrink-0 items-center gap-3 sm:gap-5">
            <span className="hidden rounded-full border border-accent-foreground/15 bg-accent px-2.5 py-1 text-[10px] font-semibold tracking-wide text-accent-foreground md:inline-flex">
              DEMO
            </span>

            <div className="flex items-center gap-2.5 sm:border-l sm:pl-5">
              <span
                className="hidden size-9 items-center justify-center rounded-full bg-secondary text-xs font-semibold sm:flex"
                aria-hidden="true"
              >
                {user.name
                  .split(" ")
        ...
        <main
          id="main-content"
          tabIndex={-1}
          className="mx-auto w-full max-w-[1500px] flex-1 p-5 outline-none sm:p-8 lg:p-10"
        >
